...
public class CommentsController {
	...
	public ModelAndView comments_list(@PathVariable("rno") int rno, @PathVariable int curPage, ModelAndView mav,
			HttpSession session) {
	...
	public ModelAndView list(@RequestParam int rno, @RequestParam(defaultValue = "1") int curPage, ModelAndView mav,
			HttpSession session) {
		int count = commentsService.count(rno);
		Pager pager = new Pager(count, curPage);
	...
	public @ResponseBody List<CommentsDTO> list_json(@RequestParam(defaultValue = "1") int curPage,
			@RequestParam int rno, HttpSession session) {
		int count = 10;
		Pager pager = new Pager(count, curPage);
		int start = pager.getPageBegin();
		int end = pager.getPageEnd();
